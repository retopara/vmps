...
while (my $line = <$inputf>)
{
    ...
    if ($line =~ /^J2Y/) {$ln_j2y = $.;}
    if ($line =~ /^J2Z/) {$ln_j2z = $.;}
    if ($line =~ /^SPLITJ1X/) {$ln_spj1x = $.;}
    if ($line =~ /^SPLITJ1Y/) {$ln_spj1y = $.;}
    if ($line =~ /^SPLITJ1Z/) {$ln_spj1z = $.;}
    if ($line =~ /^HX/) {$ln_hx = $.;}
    if ($line =~ /^HY/) {$ln_hy = $.;}
    if ($line =~ /^HZ/) {$ln_hz = $.;}
    if ($line =~ /^TOL/) {$ln_tol = $.;}
...
defined ($ln_spj1x) or die "Cound not find line start with SPLITJ1X: $!";
...
foreach my $curr_v (@v)
{
    ...
    while ($i++ < $num)
    {
        ...
        #  modify SPLITJ1X 
        my @new_cont_spj1x = split (/\s+/, $lines[$ln_spj1x-1]);
        $new_cont_spj1x[1] = "$curr_spj1x\n";
        $lines[$ln_spj1x-1] = join ("      ", @new_cont_spj1x);
        print "$lines[$ln_spj1x-1]";
